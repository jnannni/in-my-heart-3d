...
const aboutMe = [
  ...
  {
    event: 'Master Thesis Intern, UX',
    location: 'Rober Bosch, Renningen',
...
const softSkills = [
  {
    info: 'Speaking business-fluent English',
    coord: [-2.5, -1.6, -6.5] as [number, number, number],
  },
  {
    info: 'My mother tongue is Russian',
    coord: [-1.4, 1, -6.5] as [number, number, number],
  },
  {
    info: 'Speaking B1 German',
    coord: [0.9, 0.5, -6.5] as [number, number, number],
  },
  { info: 'Team-player', coord: [2, 1.5, -6.5] as [number, number, number] },
  {
    info: 'Creativity is my second name',
    coord: [1.4, -0.5, -6.5] as [number, number, number],
  },
  {
    info: 'Fast learner',
    coord: [-2, 0.3, -6.5] as [number, number, number],
  },
];
...
const design = [
  ...
  {
    name: 'Usability Texting',
    coord: [-2, 0.3, -6.5] as [number, number, number],
  },
  {
    name: 'Design Thinking',
    coord: [-3, -1, -6.5] as [number, number, number],
  },
];

const projects = [
  {
    name: 'Mars Survival Index',
    description: '',
    skills: 'Typescript, React, HTML, CSS, Figma',
    link: 'https://jnannni.github.io/mars-survival-index/',
  },
  {
    name: 'Concept Design of Smart Home app',
    description: '',
  ...
  },
  {
    name: 'Hanoi Tower',
    description: '',
    skills: 'TypeScript, HTML, CSS, PixelArt',
    link: 'https://github.com/jnannni/Hanoi_Tower/deployments/github-pages',
  },
];
...
const Carousel = () => {
  ...
  const prevCameraPos = useRef(new THREE.Vector3(0, 0, -15));
  ...
  useEffect(() => {
    if (currentPortal !== '') {
      ...
      controlsRef.current?.getPosition(prevCameraPos.current);

      const portal = scene.getObjectByName(currentPortal);
      if (portal) {
        portal.getWorldPosition(targetPosition);
        const portalDirection = targetPosition
          .clone()
          .normalize()
          .multiplyScalar(0.1);
      ...
      controlsRef.current?.setLookAt(
        prevCameraPos.current.x,
        0,
        prevCameraPos.current.z,
        0,
        0,
        0,
        true,
      ...
      {Object.entries(skillSet).map(([key, value], i) => {
        return (
          <group
            key={key}
            position={[
              Math.sin((i / skillSetLength) * Math.PI * 2) * radius,
              0,
              Math.cos((i / skillSetLength) * Math.PI * 2) * radius,
            ]}
